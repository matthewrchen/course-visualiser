...
export default function Tree({ specName }: Props) {
    ...
    const [courseDetails, setCourseDetails] = useState<null | (Course & { type: string })>(null);

    useEffect(() => {
        fetch("/api/courses").then(res => res.json()).then((allCourses: Course[]) => {
            const visibleCourses = allCourses.filter(c => c.spec[specName] || c.spec["PartI"]);
